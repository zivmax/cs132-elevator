...
class TestFloorValidation:
    ...
    def test_invalid_floors_below_minimum(self):
        ...
        for floor in invalid_floors:
            assert not validate_floor(floor), f"Floor {floor} should be invalid (below minimum)"
    
    def test_invalid_floors_above_maximum(self):
        """Test that floors above maximum are rejected"""
        invalid_floors = [4, 10, 100]
        for floor in invalid_floors:
            assert not validate_floor(floor), f"Floor {floor} should be invalid (above maximum)"
    ...
    def test_boundary_conditions(self):
        ...
        assert not validate_floor(MIN_FLOOR - 1), "One below MIN_FLOOR should be invalid"
        assert not validate_floor(MAX_FLOOR + 1), "One above MAX_FLOOR should be invalid"
...
class TestElevatorIdValidation:
    ...
    def test_valid_elevator_ids(self):
        ...
        for elevator_id in valid_ids:
            assert validate_elevator_id(elevator_id), f"Elevator ID {elevator_id} should be valid"
    ...
    def test_invalid_elevator_ids_below_minimum(self):
        ...
        for elevator_id in invalid_ids:
            assert not validate_elevator_id(elevator_id), f"Elevator ID {elevator_id} should be invalid (below minimum)"
    
    def test_invalid_elevator_ids_above_maximum(self):
        """Test that elevator IDs above maximum are rejected"""
        invalid_ids = [3, 10, 100]
        for elevator_id in invalid_ids:
            assert not validate_elevator_id(elevator_id), f"Elevator ID {elevator_id} should be invalid (above maximum)"
    ...
    def test_elevator_id_boundary_conditions(self):
        ...
        # Test just outside boundaries
        assert not validate_elevator_id(MIN_ELEVATOR_ID - 1), "One below MIN_ELEVATOR_ID should be invalid"
        assert not validate_elevator_id(MAX_ELEVATOR_ID + 1), "One above MAX_ELEVATOR_ID should be invalid"
...
class TestDirectionValidation:
    ...
    def test_valid_directions(self):
        ...
        for direction in valid_directions:
            assert validate_direction(direction), f"Direction '{direction}' should be valid"
    ...
    def test_invalid_directions(self):
        ...
        for direction in invalid_directions:
            assert not validate_direction(direction), f"Direction '{direction}' should be invalid"
    ...
    def test_direction_case_sensitivity(self):
        ...
        for direction in case_variants:
            assert not validate_direction(direction), f"Direction '{direction}' should be invalid (case sensitive)"
    ...
    def test_direction_whitespace(self):
        ...
        for direction in whitespace_variants:
            assert not validate_direction(direction), f"Direction '{repr(direction)}' with whitespace should be invalid"
...
class TestValidationIntegration:
    ...
    def test_manual_verification_scenario(self):
        ...
        assert result_valid == True, "Floor 2 should be valid (within MIN_FLOOR=-1, MAX_FLOOR=3)"
        
        # Invalid floor outside bounds  
        result_invalid = validate_floor(5)
        assert result_invalid == False, "Floor 5 should be invalid (exceeds MAX_FLOOR=3)"
        
        # Verify the exact output matches manual test: True False
        assert (result_valid, result_invalid) == (True, False), "Manual verification should return (True, False)"
    ...
    def test_edge_case_combinations(self):
        ...
        assert not (validate_floor(MIN_FLOOR - 1) and validate_elevator_id(MIN_ELEVATOR_ID))
        assert not (validate_floor(MAX_FLOOR + 1) and validate_elevator_id(MAX_ELEVATOR_ID + 1))
