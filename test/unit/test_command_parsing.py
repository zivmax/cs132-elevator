...
class TestCommandValidation:
    """Test cases for command validation and boundary checking"""
    ...
    def test_parse_boundary_elevator_ids(self):
        """Test parsing commands with boundary elevator IDs"""
        # Test minimum elevator ID
        result_min = self.coordinator._parse_message_to_command("open_door#1")
        assert isinstance(result_min, OpenDoorCommand)
        assert result_min.elevator_id == 1

        # Test maximum elevator ID
        result_max = self.coordinator._parse_message_to_command("close_door#2")
        assert isinstance(result_max, CloseDoorCommand)
        assert result_max.elevator_id == 2

    def test_parse_various_call_directions(self):
    ...
    def test_parse_multiple_commands_integrity(self):
        """Test that parsing multiple commands maintains integrity"""
        commands = [
            "call_up@1",
            "select_floor@2#1",
            "open_door#1",
        ...
        assert isinstance(results[2], OpenDoorCommand)
        assert isinstance(results[3], CloseDoorCommand)
        assert isinstance(results[4], ResetCommand)

    def test_parse_error_commands_integrity(self):
        """Test that error commands are handled consistently"""
        error_commands = [
            "",
            "invalid_command",
            "call_up",
